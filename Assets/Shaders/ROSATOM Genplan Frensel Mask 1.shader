Shader "ROSATOM Genplan Frensel Mask" {
Properties {
	_Color ("Main Color", Color) = (1,1,1,1)
	_MainTex ("Base (RGB) Trans (A)", 2D) = "white" {}
	_Alpha ("Base (RGB) Trans (A)", 2D) = "white" {}
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	fixed4 mask = tex2D(_Alpha, IN.uv2_Lightmap);
	fixed4 c = tex * _Color;
	c *= lm  * _Brightness  ;
	half rim = abs(0.25 - dot (normalize(IN.viewDir), o.Normal))  ;
	o.Albedo = c.rgb * pow(rim,(-_Shininess));
	o.Alpha = mask * _Color.a;
